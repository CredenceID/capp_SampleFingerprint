...
import com.credenceid.biometrics.Biometrics.FingerprintReaderStatusListener;
...
public class FingerprintActivity
		extends Activity {
	private final static String mTAG = FingerprintActivity.class.getSimpleName();
	...
	private FingerprintReaderStatusListener mFingerprintOpenCloseListener =
			new FingerprintReaderStatusListener() {
				@Override
				public void onOpenFingerprintReader(ResultCode resultCode,
													String hint) {
					Log.d(mTAG, "onOpenFingerprintReader(): " + resultCode.name());
	...
	captureFingerprintOne() {
		...
		mBiometricsManager.grabFingerprint(mScanTypes[0], new OnFingerprintGrabbedWSQListener() {
			@SuppressLint("SetTextI18n")
			@Override
			public void onFingerprintGrabbed(ResultCode resultCode,
											 Bitmap bitmap,
											 byte[] bytes,
											 String filepath,
											 String wsqFilepath,
	...
	captureFingerprintTwo() {
		...
		mBiometricsManager.grabFingerprint(mScanTypes[0], new OnFingerprintGrabbedListener() {
			@Override
			public void onFingerprintGrabbed(ResultCode resultCode,
											 Bitmap bitmap,
											 byte[] bytes,
